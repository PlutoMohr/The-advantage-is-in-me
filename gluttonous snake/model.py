...
class BDMSNN(nn.Module):
    def __init__(self, num_state, num_action, weight_exc, weight_inh, node_type):
        ...
        out_shape = [self.connection[0].weight.shape[1], self.connection[1].weight.shape[1],
                     self.connection[2].weight.shape[1], self.connection[4].weight.shape[1],
                     self.connection[3].weight.shape[1], self.connection[10].weight.shape[1],
                     self.connection[11].weight.shape[1], self.connection[9].weight.shape[1]]
        self.out = []
        self.dw = []
    ...
    def UpdateWeight(self, i, s, num_action, dw):
        ...
        if self.node_type == "hh":
            ...
            self.connection[i].weight.data[s, [s * num_action, s * num_action + 1]] /= (
                    self.connection[i].weight.data[s, [s * num_action, s * num_action + 1]].float().max() + 1e-12)
        ...
        if self.node_type == "lif":
            ...
            dw[s, [s * num_action, s * num_action + 1]] = (dw[s, [s * num_action,
                                                                  s * num_action + 1]] - dw_mean) / dw_std
    ...
    def chooseAct(self, input, weight_trace_d1, weight_trace_d2):
        for i_train in range(500):
            ...
            if torch.max(out) > 0:
                return torch.argmax(out, dim=1), weight_trace_d1, weight_trace_d2, self

    def updateNet(self, reward, action, state, weight_trace_d1, weight_trace_d2):
        r = torch.ones((n_state, n_state * n_action), dtype=torch.float)
        r[state, state * n_action + action] = reward
        dw_d1 = r * weight_trace_d1
        dw_d2 = -1 * r * weight_trace_d2
        self.UpdateWeight(0, state, n_action, dw_d1)
        self.UpdateWeight(1, state, n_action, dw_d2)
        return self
